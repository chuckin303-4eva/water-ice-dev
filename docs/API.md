...
| POST | `/locations` | Create a location/prospect. Requires `address`, or `latitude`+`longitude`, or both -- whichever is missing is filled in by geocoding (Nominatim). Returns `202` + `ValidationQueueResponse` instead of `201` + `LocationResponse` if the org requires review and the caller isn't an admin (ADR-0014) | Yes |
