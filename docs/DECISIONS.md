...
## ADR-0017: Opportunity scoring, refined — product-overlap competition and reactive recalculation

Date: 2026-07-29
Status: Accepted

### Context
Phase 2, third item. Basic scoring (ADR-0009) left two gaps it explicitly flagged as deferred rather than fixed: `competition_score` counted every nearby competitor regardless of what they actually served (a water-only rival counted fully against an ice-only prospect), and scores went stale whenever competitor data changed nearby — only editing the location itself or calling the manual `POST /locations/{id}/recalculate-score` refreshed them. Wiring in the still-unused demographic fields (`population`/`median_income`/`growth_rate`) was considered and rejected for this pass — that requires a real external data source, which is the Market Refresh Engine's job (ADR-0004, Phase 3), and pulling it forward here would jump a phase boundary for a "refined scoring" ask that didn't require it. Both gaps actually fixed here are pure backend logic against data already in the schema — no new infrastructure, no new cost.

### Decision
1. **`competition_score` only counts competitors that share a product the location has actually declared**, once it's declared one. `calculate_competition_score` gains `serves_ice`/`serves_water` parameters; if the location has at least one set, a competitor only counts if it shares at least one of those same capabilities. This is the same opt-in-narrowing rule already established for filters (ADR-0010): a capability flag defaulting to `False` on a fresh record must narrow only when explicitly turned on, never exclude by default. A brand-new, unconfigured prospect (both flags still `False`) keeps the original behavior — every nearby competitor counts, unnarrowed — so a fresh prospect doesn't silently read as "no competition" just because nobody has said what it serves yet; the "0 is a confident answer" guarantee from ADR-0009 is preserved exactly for that case.
2. **Competitor writes now recalculate every location within the same 10-mile radius the scoring already uses.** New `scoring_service.recalculate_scores_near(db, latitude, longitude)`, called from `competitor_service.create_competitor` (at the new competitor's position), `update_competitor` (at *both* the old and new position, since a moved competitor can stop mattering to locations near its old spot and start mattering near its new one), and `delete_competitor` (captured before the row is gone). This directly closes the staleness gap ADR-0009 named as a known, accepted limitation rather than a bug — it no longer needs accepting.
3. **The manual `POST /locations/{id}/recalculate-score` endpoint stays.** Reactive recalculation covers the common cases (a competitor changed); it doesn't cover every conceivable edge case (e.g. a future bulk data-fix script that writes directly to the database), so the on-demand escape hatch remains rather than being removed as "no longer needed."
4. **`confidence_score`'s meaning is unchanged.** It still reflects only whether `visibility_rating`/`traffic_score` are present — the product-overlap narrowing changes what `competition_score` *counts*, not whether the inputs feeding `opportunity_score` are complete, so conflating the two would blur a metric that was already correct.

### Alternatives considered
- **Wiring a free demographic source (e.g. US Census) into the formula now**: rejected — that's explicitly the Market Refresh Engine's scope (ADR-0004, Phase 3), gated behind Phase 2 finishing; doing it here would jump the phase order for a request that didn't need it.
- **Making an unconfigured location's competition_score default to `None`/unscored instead of counting everything**: considered and rejected in favor of the opt-in-narrowing framing — `0` competitors-found and `0` because-nothing-was-declared-yet would look identical to a user, and the opt-in-narrowing rule is both simpler and already the established pattern elsewhere in this codebase.
- **Only recalculating on competitor create, not update/delete**: rejected — a moved or removed competitor is exactly as capable of leaving a location's score stale as a new one appearing; covering only creation would have been a partial, confusing fix.
- **Redefining `confidence_score` to also reflect nearby-data completeness**: rejected — no clear, non-arbitrary definition of "how complete is the competitive picture" exists, versus the crisp, already-correct "are the two manual ratings present" it currently measures.

### Consequences
- Every competitor create/update/delete now does extra work proportional to how many locations sit within its 10-mile radius (a bounding-box query plus one `recalculate_scores` call per match) — a real, accepted cost at today's scale, consistent with this project's "don't design for scale that doesn't exist yet" pattern; revisit only if it's ever measured as a problem.
- A location's `competition_score` can now change without that location being touched at all (a nearby competitor write did it) — expected and desired, but worth remembering if `update_log`/audit expectations for `locations` are ever revisited, since these score changes are not currently logged there (matching how score recalculation from the location's own edits was never logged either — computed fields, not manually-entered facts).
- `calculate_competition_score`'s `serves_ice`/`serves_water` parameters default to `False` for backward compatibility with existing direct callers (tests); every real call site (`recalculate_scores`) always passes the location's actual values.
- 7 new backend tests (134 total): product-overlap narrowing at the scoring-function level, plus create/update/delete reactive recalculation exercised through the real `/competitors` API. Verified end-to-end against the live backend via curl: creating a same-product competitor near an already-scored location raised its `competition_score` and lowered its `opportunity_score` with no manual recalculation call; deleting that competitor reverted it; a water-only competitor never affected an ice-only location's score.

---

## ADR-0018: Photos — local disk storage, unauthenticated media serving, scoped to locations and competitors

Date: 2026-07-30
Status: Accepted

### Context
Phase 2, fourth and final item. `photos` (polymorphic `entity_type`/`entity_id`, `file_key`, `caption`, `uploaded_by`, `uploaded_at`, `is_primary`) was designed in ADR-0003 alongside the rest of the original schema but never built — no model, no migration, no storage infrastructure, nothing. This is the first feature in this project needing actual binary file storage rather than just database rows, so the storage-backend decision (local disk vs. S3/cloud) had to be made for real, not assumed. The sibling `local-presence-command` (LPC) project already faced and resolved this exact question for its own "media library" feature (its own ADR-0002: local disk for MVP), including image compression, EXIF stripping, content-type verification, and static-file serving — reused directly rather than re-litigated from scratch.

### Decision
1. **Local disk storage, not S3**, mirroring LPC's own MVP decision exactly: files land under `settings.upload_dir` (default `uploads/`, overridable via env), served back out through a `StaticFiles` mount at `/media`. This is a real, accepted limitation, not an oversight: files will not survive redeployment onto an ephemeral filesystem, and this project's backend isn't even containerized yet (`infra/docker-compose.yml` only defines `postgres`) — there's no volume-mount question to answer today. Revisit with a new ADR (and likely a real migration script to move existing files) once the backend is actually deployed somewhere that matters, not before.
2. **Pillow for compression, mirroring LPC's proven approach**: every upload is downscaled to at most 2048px per side (never upscaled), re-encoded at quality 85, and EXIF is stripped by simply not carrying it through on save (privacy — phone photos often embed GPS coordinates). Decoding the image via Pillow doubles as content-type verification: a file whose declared `Content-Type` doesn't match its actual bytes fails to decode and is rejected (422), rather than being trusted at face value.
3. **Scoped to `locations` and `competitors` only, not the full polymorphic set** (`host_businesses`/`brands` are also UUID-keyed and could technically take a photo per the original schema design). Both of those have no detail panel or any consuming UI at all — building photo upload for entities nobody can currently view photos on would be pure unused scaffolding. The `entity_type` column stays genuinely polymorphic in shape (no schema change needed to add a third type later); `photo_service._ENTITY_MODELS` is the single place that would need a new entry.
4. **Column renamed from the original design's `file_key` to `file_url`, storing an already-servable path** (e.g. `/media/location/<uuid>.jpg`), not a raw storage key the caller would need to reconstruct a URL from. Same reasoning LPC already applied: swapping the storage backend later (e.g. to S3) means updating what gets written to this column, not every consumer that reads it — and it means the frontend can put the value directly into `<img src>` with just the API's base URL prefixed, no extra path-joining logic.
5. **Media is served unauthenticated.** `StaticFiles` doesn't run through `get_current_user` — anyone with a photo's exact URL can view it, protected only by the URL containing an unguessable server-generated UUID filename, not real access control. This is a deliberate, disclosed tradeoff (`<img>` tags can't attach an `Authorization` header without extra JS-driven blob-fetching machinery this feature doesn't need yet), the same class of pre-public-facing-tool tradeoff already accepted for `localStorage` token storage (ADR-0007). Revisit before this product is customer-facing.
6. **Nested routes per parent** (`POST/GET /locations/{id}/photos`, `DELETE /locations/{id}/photos/{photo_id}`, and the same shape under `/competitors`), not a flat `/photos?entity_type=&entity_id=` resource — matches the existing nested-sub-resource convention already used for call notes, backed by one shared, entity-agnostic `photo_service.py` so the actual upload/validation/compression logic isn't duplicated per parent.
7. **`is_primary` is supported end-to-end at the API layer (uploading a new primary photo unsets any previous one for that entity; the gallery sorts primary-first) but has no upload-time UI control in v1** — no checkbox in `PhotoGallery`. Documented as a real, narrow gap rather than silently dropped, same pattern as ADR-0015's "no unlink affordance yet."

### Alternatives considered
- **S3 or another cloud object store from day one**: rejected — a new paid dependency with no current deployment target to justify it, and this project's established cost-averse default requires explicit sign-off before adding one. Local disk is free and already proven at this same stage of the sibling project.
- **Keeping the original `file_key` naming, having the frontend construct the full path**: rejected — an opaque, already-complete URL is simpler for every consumer and matches LPC's already-validated approach.
- **A flat, single `/photos` endpoint filtered by query params**: rejected for consistency — every other sub-resource in this API (call notes, calendar links) is nested under its parent, and inventing a different shape just for photos would be a needless inconsistency.
- **Building photo support for `host_businesses`/`brands` too, since the schema allows it**: rejected as unused scaffolding — no detail panel exists for either today.
- **Requiring authentication to view a photo (e.g. a signed-URL scheme)**: rejected for v1 as more machinery than currently justified; the unguessable-UUID-filename tradeoff is disclosed here rather than silently accepted.

### Consequences
- `backend/uploads/` is gitignored, matching the established pattern for other non-source, user-generated artifacts (downloaded CSV exports, ADR-0013).
- New runtime dependency: `Pillow` (image decoding/compression).
- Deleting a photo removes both the database row and the file from disk; a missing file on disk is tolerated (logged as a no-op, not a failure) rather than blocking the delete, since the DB row going away is the more important guarantee.
- This closes out every item on the Phase 2 roadmap (validation workflow, host businesses, opportunity scoring refined, photos) — Phase 3 (Subscriptions/Billing/Market Refresh Engine) is next per ROADMAP.md.
- 11 new backend tests (145 total): upload for both entity types, content-type rejection, a spoofed-Content-Type file that fails to actually decode, oversized-file rejection, list, delete (row and file both gone), `is_primary` unsetting the previous primary, and cross-entity/cross-type 404 protection on delete. Verified end-to-end against the live backend via curl (the embedded browser's screenshot/accessibility-tree tooling was non-functional this session, unrelated to this feature): upload returned a real compressed JPEG smaller than the original, the media URL served it back with the correct `Content-Type`, and delete removed it from both the database and disk.

---

## ADR-0019: Subscriptions & billing — mock payment provider, no plans table

Date: 2026-07-30
Status: Accepted

### Context
Phase 3, first items ("Subscriptions" and "Billing" — the actual revenue mechanism per ROADMAP.md's own framing). User asked to be able to test the full billing flow (plans, subscribe, switch, cancel, invoice history) without signing up for a real payment processor (Stripe or otherwise) first. Nothing billing-related existed anywhere in the schema or docs before this — fully greenfield, unlike every other Phase 1/2 feature which had at least a schema sketch from ADR-0002/0003 to build on.

### Decision
1. **A `BillingProvider` protocol, same replaceable-module shape as the Market Refresh Engine's `MarketDataProvider` (ADR-0004)** — `create_subscription`/`cancel_subscription` behind one interface, so a real processor can be swapped in later without touching the subscribe/cancel/invoice orchestration in `billing_service.py`. Only `MockBillingProvider` exists today; adding a real one is a new ADR plus explicit sign-off before it becomes the active provider — the same paid-integration gating precedent ADR-0004 already established for demographic/rating data sources, applied here to payments.
2. **`MockBillingProvider` collects no real payment method at all — not even a fake credit-card form.** Every subscribe/switch action succeeds immediately, generates a fake `provider_subscription_id`, and is marked paid. This matches this project's repeated refusal to fake something that isn't actually built (no fake "invite sent" emails in ADR-0012, no fabricated competitor data in ADR-0008) — a card-entry form that doesn't actually charge anything would be exactly that kind of dishonest UI. The billing page instead shows a plain, visible "Demo mode" banner stating no real processor is connected.
3. **No `plans` database table.** Pricing tiers (`free`/`starter`/`pro` — slug, name, price, feature bullets) are a fixed catalog in `app/services/plan_catalog.py`, referenced by `slug` string, not a foreign key. Pricing is a business/deploy-time decision, not user-editable data — this app has no admin UI for managing arbitrary reference data, and building one solely so pricing could theoretically change without a code deploy would be premature flexibility for a catalog of three rows.
4. **`subscriptions` is one upserted row per organization; `invoices` is the append-only historical record** — mirrors how real billing systems (Stripe included) separate a mutable "current subscription state" object from immutable historical invoice/charge records, rather than trying to preserve every past subscription state as separate rows. Absence of a `subscriptions` row means the organization is on the free plan — same "opt-in, default is the off/free state" convention already used for `require_review_for_submissions` (ADR-0014) — so no migration or backfill was needed for organizations that existed before this feature.
5. **Invoices snapshot `plan_slug` and `amount_cents` at issue time** rather than joining live against the plan catalog, so a future price change in code never silently rewrites what a past invoice says was actually charged.
6. **Cancellation is immediate, not at-period-end.** Most real billing platforms default to letting a canceled subscription run out its paid period first, which needs either a background job or a lazy-expiry check at read time to flip the status once the period passes. Neither exists in this app (deliberately, per ADR-0004's no-job-queue stance), and building either just for this would be more machinery than needed to prove the core subscribe/switch/cancel/invoice flow. Documented as a real, simpler-than-typical behavior, not an oversight — revisit if a grace period is ever actually requested.
7. **No proration on a plan switch.** Switching from Starter to Pro mid-period immediately creates a new invoice for Pro's full price, with no credit for unused Starter time. Real proration math is genuinely complex and orthogonal to what's being tested here (can the flow be exercised end to end); a simplification worth naming, not silently hidden.
8. **`GET /billing/subscription` is open to any authenticated org member (read-only status, same as organization settings); `subscribe`/`cancel`/`GET /billing/invoices` are admin-only.** Invoice history is treated as more sensitive than a plain current-plan readout (it's financial history), so unlike the user roster (open to any member) it's gated the same as the mutating actions.

### Alternatives considered
- **Wiring in a real processor (Stripe test mode) from the start**: rejected — the user explicitly asked to test this without signing up for anything yet; Stripe's test mode still requires an account and API keys, which is exactly the friction being avoided.
- **A `plans` database table with an admin CRUD UI**: rejected as premature — no current need to change pricing without a deploy, and no other reference data in this app gets a bespoke admin-editable table for a fixed handful of rows (host businesses/brands are genuinely open-ended user data, not a 3-row pricing catalog).
- **A fake payment-method form that doesn't actually validate or store anything**: rejected outright — indistinguishable from real payment collection to whoever's testing it, and this project has consistently avoided exactly this kind of misleading simulation elsewhere.
- **Preserving every historical subscription state as its own row** (never updating `subscriptions` in place): rejected — `invoices` already is that historical record for what was actually charged; versioning the subscription row too would duplicate that guarantee for no added benefit.
- **Cancel-at-period-end with a lazy-expiry check on read**: considered (and the schema was briefly sketched with a `cancel_at_period_end` column) but dropped for v1 simplicity — immediate cancellation proves the same flow with less moving logic to verify.

### Consequences
- This is a **testable prototype of the billing UX and data model, not a live revenue mechanism** — no money actually moves. Anyone reading ROADMAP.md's "Critical — this is the actual revenue mechanism" framing for this line item should not mistake this for done-and-collecting; a real provider integration remains a distinct future decision requiring explicit sign-off (point 1 above).
- Switching the active provider later means changing what `billing_providers.get_active_provider()` returns (and building the real provider's `create_subscription`/`cancel_subscription` behind the same interface) — `billing_service.py`'s orchestration logic shouldn't need to change.
- No usage metering or plan-limit enforcement exists — a plan's `features` list (e.g. "Up to 250 locations") is purely descriptive/display text today, not something the app actually checks or blocks against. Building real limit enforcement is a distinct, unscoped feature, not implied by this one.
- 10 new backend tests (155 total): free-by-default for a fresh org, plan catalog listing, subscribe creates a subscription and an invoice, switching plans updates the subscription and adds a second invoice, cancel reverts to free while preserving invoice history, canceling with nothing active is rejected (409), subscribing to `free` directly or to an unknown plan is rejected (422), and a non-admin can view plans/current-subscription but not subscribe/cancel/view invoices (403). Verified end-to-end in a real browser: subscribed to Starter, switched to Pro, canceled back to Free, with the current-plan card, plan-card button states, and invoice history table all updating correctly at each step with no page reload.

## ADR-0020: Market Refresh Engine — free-tier providers, scope narrowed from ADR-0004's original design

Date: 2026-07-30
Status: Accepted

### Context
Phase 3 item, and the last of ADR-0004's original design still unbuilt. ADR-0004 (Phase 1) had sketched `Refresh → Compare → validation_queue → human approval → update_log`, a `MarketDataProvider` protocol, and a v1 provider scope covering address changes, new competitor POIs, closed/moved businesses, host-tag changes, and duplicate detection — all via OpenStreetMap. That sketch predated this project's actual OSM research (ADR-0008) and the schema decisions made since (validation_queue's polymorphic shape in ADR-0014, `submitted_by` nullability). Building it now meant reconciling the two-year-old plan with what's actually true today, not just implementing it as originally written.

### Decision
1. **v1 scope is address-drift (via the existing Nominatim reverse-geocoding already used elsewhere in this app) plus US Census demographics (population, median household income, derived growth rate) — nothing else from ADR-0004's original list.** New-competitor-POI detection was dropped because ADR-0008 already found, via direct research, that OSM has essentially no tagging coverage for ice/water vending machines — building an Overpass query against that data is a known near-zero-yield feature, not a hypothetical risk worth hedging against. Closed/moved-business and host-tag-changed detection were dropped because both need a stable OSM node ID captured at location-creation time to track a specific POI across refreshes, and the schema doesn't have one — deferred pending that groundwork, not built as a guess. Duplicate-location detection was dropped because "flag this as a possible duplicate" has no defined resolution/merge workflow on either the validation-queue or location model; shipping the flag without a way to act on it would be a half-built feature.
2. **US Census integration uses two endpoints with different auth requirements, discovered against the real API, not assumed up front.** The Census Geocoder (coordinates → state/county/tract FIPS) is keyless. The ACS 5-Year Estimates API (population via `B01003_001E`, median income via `B19013_001E`) 302-redirects to an error page without a key — the original "free, no key needed" assumption in the first draft of `census_service.py` was simply wrong, caught by testing against the live API rather than trusting the mocked unit tests alone. A free key (`CENSUS_API_KEY`, signup at https://api.census.gov/data/key_signup.html, no cost) is now required for the Census provider; `get_demographics` raises `CensusLookupError` when it's unset, which `CensusProvider.check_location` treats the same as any other lookup failure — that location is skipped for this run, not a failure of the whole refresh. Growth rate is derived (not a single Census field) by comparing population across two ACS5 vintages (`CURRENT_ACS_YEAR`/`EARLIER_ACS_YEAR`, bumped periodically as new releases ship); a tract with no data for the earlier vintage returns HTTP 204 with an empty body rather than an HTTP error or a JSON array — also discovered live, not documented behavior assumed in advance — and is now handled explicitly rather than crashing on `.json()`.
3. **One combined `validation_queue` proposal per location per run, not one per field.** When a location has both an address change and demographic drift in the same run, `market_refresh_service.run_refresh` merges every `FieldObservation` into a single proposal with a combined `reason` string, so a reviewer sees one card per location instead of a flood of near-duplicate cards for the same site.
4. **System-sourced proposals have `submitted_by=None`**, extending the pattern ADR-0014 already established for that column being nullable, and are approved with `change_source="verification"` (a new `update_log` value alongside `manual`/`import`/`system`) so the audit trail honestly reflects how a change was actually discovered.
5. **This forced two real bug fixes in already-shipped Phase 2 code, both the same root cause**: `validation_service.list_queue()` used an inner `JOIN` on `User` via `submitted_by`, which silently excluded every system-sourced entry (`submitted_by IS NULL`) from every admin's queue view — the entire refresh engine's output would have been invisible. `validation_queue.py`'s `_get_entry_or_404()` had the identical bug in its cross-org ownership check, which (even after the first fix) would have 404'd every attempt to approve or reject a system-sourced entry. Both are fixed the same way: a `submitted_by IS NULL` entry is visible to and actionable by every organization's admins, because it concerns shared, platform-wide location data (ADR-0002), not one tenant's private submission. Existing member-submitted entries remain scoped to their own org exactly as before — verified with a regression test, not just the new-path tests.
6. **Synchronous and bounded (`MAX_LOCATIONS_PER_RUN=20`), triggered by an admin clicking a button** — no background job queue or scheduler, matching ADR-0004's original no-new-infrastructure stance. A run can take up to roughly a minute; the frontend button disables itself and shows a "this can take up to a minute" label rather than pretending it's instant.
7. **`population`/`median_income`/`growth_rate` are refresh-engine-populated only** — no manual-entry UI exists for them, even though `LocationUpdateRequest` accepts them (so a human could technically `PUT` them directly via the API). Nothing in the product prompts for that today; adding manual entry is a distinct, unscoped decision.

### Alternatives considered
- **Keeping ADR-0004's full original provider list**: rejected — see point 1; two of the four dropped checks are known-empty given ADR-0008's findings, and the other two need schema groundwork (a stable OSM node ID) this project doesn't have yet.
- **One `validation_queue` entry per changed field**: rejected — reviewer ergonomics; a location with three drifted fields would otherwise produce three near-identical cards to review and approve/reject independently, when they're really one "recheck this location" event.
- **A background job/scheduler for periodic automatic refreshes**: rejected for the same reason ADR-0004 rejected it originally — no job infrastructure exists in this app, and an admin-triggered synchronous run is sufficient to prove the value before investing in that infrastructure.
- **Trusting only the mocked Census/OSM unit tests as "done"**: rejected — mocked tests only prove the code handles the response shape assumed going in, not that the real API actually returns that shape. Verifying against the live Census Geocoder and ACS5 endpoints caught two real defects (the missing-key redirect, the 204-empty-body case) that no amount of testing against my own assumptions would have found.

### Consequences
- A `CENSUS_API_KEY` is now a required piece of local/deploy configuration for the Census half of this feature to function; without it, the Census provider silently no-ops (skips every location) rather than erroring the whole run, and the OpenStreetMap address-drift provider still works unaffected. Documented in `.env.example`.
- Extending the Market Refresh Engine to a new signal later (e.g. once a stable OSM node ID exists) means adding a new `MarketDataProvider` to `market_refresh_providers.PROVIDERS` — `market_refresh_service.py`'s orchestration, the combined-proposal logic, and the cross-org visibility fixes all already generalize to more providers without further changes.
- 21 new backend tests (179 total, up from 155 before Phase 3 billing): 8 for `census_service` (geocoder parsing, tract-not-found, growth-rate computation, sentinel-as-missing handling, missing-earlier-vintage survival, HTTP-error wrapping, missing-API-key behavior, the 204-empty-body case) and 15 for the provider/orchestration/route layer (OpenStreetMapProvider, CensusProvider, combined-proposal creation, no-drift queues nothing, archived locations are skipped, oldest-or-never-checked locations are prioritized, the two cross-org validation-queue regression tests, admin-only route gating, trigger-then-appears-in-history). Verified end-to-end against the real, live backend (not just pytest): created a real location, ran a refresh that hit the actual Census Geocoder/ACS5 and Nominatim APIs, confirmed the resulting system-sourced proposal was visible to an admin from a different organization who submitted nothing, approved it, and confirmed the location's `address`/`population`/`last_verified_at`/`verification_source` updated correctly with `update_log.change_source="verification"` and `changed_by=NULL`. Also verified the Refresh admin page renders real run history in a live browser session.

## ADR-0021: Advanced Analytics — a prerequisite `opportunities` table, then aggregation with no new cost

Date: 2026-08-03
Status: Accepted

### Context
Phase 4's first item, "Advanced analytics." Before designing what to build, I checked what data existed to analyze: `opportunities` (the pursuit-workflow layer distinct from a location's computed `opportunity_score` -- who's pursuing a given location, and how far along) was designed back in ADR-0003 alongside the rest of the schema, but had never actually been implemented in Phase 1, 2, or 3 -- unlike `host_businesses`/`brands`/`photos`, which had the identical "designed but unbuilt" gap and got closed when a feature actually needed them (ADR-0015/0016/0018), `opportunities` had simply never been a numbered roadmap item at all. Building "advanced analytics" without it would mean analytics over location/scoring/demographic data only, with no pipeline to show -- thin for a sales-and-prospecting tool whose whole point is tracking pursuits.

### Decision
1. **Built the `opportunities` table now, as a prerequisite, not scope creep.** Per the PM operating rule ("a request that jumps ahead of the current phase or adds scope the phase didn't call for gets flagged, with a recommended sequence, not built silently"), this was flagged to the user before starting, with the same reasoning as above: this is a pre-existing backlog gap analytics needs to be worth building, not new scope invented for this feature. `Opportunity`: `location_id` (FK → locations), `organization_id` (FK → organizations -- the same location can be pursued independently by more than one org, since `locations` itself is shared platform-wide data per ADR-0002), `stage` (`identified`/`contacted`/`negotiating`/`won`/`lost`), `assigned_user_id` (nullable FK → users), `priority`, `target_action_date`, `outcome_notes`. New `opportunity_service.py` + `/opportunities` CRUD, org-scoped (list/get/update/delete all 404 outside the caller's org), open to any authenticated member (no admin gate -- this is working data, not billing-sensitive). `assigned_user_id` is validated against the caller's own organization (422, not a raw FK error) -- assigning to a user who couldn't act on it would be a silent no-op otherwise.
2. **Location/competitor/demographic analytics are platform-wide (ADR-0002); the pipeline funnel is the one org-scoped piece.** `analytics_service.py` computes portfolio composition (status breakdown), opportunity-score distribution (bucketed + average + unscored count), top-scored prospects, fastest-growing markets (from the Market Refresh Engine's `growth_rate`, ADR-0020 -- the first feature to actually consume that data), most-contested markets (`competition_score`), and competitor totals -- all shared across every organization, same visibility model as the location data itself. The pipeline funnel (`opportunities` grouped by `stage`) is scoped to the caller's own organization, since that's a specific tenant's pursuit activity, not shared intelligence. One `GET /analytics/summary` endpoint returns all of it together rather than one endpoint per metric -- the whole point is a single dashboard view, not a metrics API surface to be composed piecemeal.
3. **No new dependency for charts.** A hand-rolled horizontal bar-list component (a labeled `<div>` sized by percentage) covers every chart this v1 needs (status breakdown, score buckets, pipeline funnel) -- adding a charting library for three simple bar lists would be premature; revisit if a future analytics view needs real chart interactions (zoom, tooltips, time series) a library would actually earn its keep for.
4. **Pure aggregation over data already collected by Phases 1-3** (scoring, ADR-0009/0017; Market Refresh demographics, ADR-0020; competitor tracking, ADR-0008) plus the new `opportunities` table -- no new external API calls, no new cost, consistent with this project's standing cost-averse default.
5. **Deferred "AI recommendations" (Phase 4's other listed item).** Any useful version of it needs a paid LLM API call per request -- an ongoing per-use cost, unlike the free/keyless-except-Census sources used everywhere else in this project. Same paid-integration gating precedent as ADR-0004/ADR-0019: requires its own ADR and explicit sign-off before being built, not bundled into this one because they're both nominally "Phase 4."

### Alternatives considered
- **Building analytics only on existing location/competitor/scoring data, skipping `opportunities` entirely**: rejected -- would ship "advanced analytics" with no pipeline view for a product whose core workflow is pursuing locations, and the missing table is a genuine backlog gap worth closing now rather than working around indefinitely.
- **A separate endpoint per metric** (`/analytics/status-breakdown`, `/analytics/score-distribution`, etc.): rejected -- the frontend always needs all of them together for one dashboard render; one combined response avoids six round-trips for a single page load.
- **Adding a charting library** (Recharts, Chart.js): rejected for v1 -- three bar lists don't need real chart infrastructure; would be premature dependency weight for what a few styled `<div>`s already do.
- **Building AI recommendations first instead of analytics**: rejected -- it requires a paid API decision that hasn't been made, while analytics needed no such sign-off and could ship immediately with zero new cost.

### Consequences
- `opportunities` existing now means any future feature needing pursuit-workflow data (assignment dashboards, follow-up reminders tied to `target_action_date`, win/loss reporting) has a real table to build on, not another "designed but never built" gap to rediscover.
- Analytics numbers will look thin on a fresh/small dataset (this is a dev database with a couple dozen locations) -- the aggregates themselves are correct and tested, but the feature's value scales with how much real data an organization has accumulated, same as any dashboard.
- Folding `population`/`median_income`/`growth_rate` into the `opportunity_score` formula itself remains a distinct, unscoped decision (still true after ADR-0020) -- this feature surfaces that data in analytics views but doesn't change how scoring is computed.
- 16 new backend tests (195 total): 9 for opportunities CRUD (create, invalid-location/stage/assignee rejection with 422, org-scoped listing, stage filtering, cross-org 404s on get/update/delete, delete) and 7 for analytics aggregates (status breakdown and totals, score distribution and bucketing, top-prospects excludes non-prospect/unscored, growth-markets excludes locations with no growth data, competitor landscape, pipeline funnel scoped correctly per organization, non-admin member can view). Verified end-to-end against the real, live backend and in a real browser: created an opportunity via curl, confirmed it appeared in `/analytics/summary`'s pipeline funnel; separately, clicked "Start pursuing this location" and changed its stage through the actual `LocationDetailPanel` UI (not just curl) and confirmed the stage change persisted to the database; confirmed the Analytics page renders live data matching the API response exactly.
