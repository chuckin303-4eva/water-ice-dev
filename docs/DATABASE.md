...
- `population` — surrounding population figure. **Implemented** (ADR-0020) — populated by the Market Refresh Engine's Census provider (US Census ACS 5-Year Estimates), refresh-engine-populated only (no manual-entry UI); deliberately still excluded from the opportunity_score formula, not defaulted to zero.
- `median_income` — "Income". Same status/source as `population`.
- `growth_rate` — "Growth". Same status as `population`; derived (not a single Census field) as the percent change in population between two ACS5 vintages five years apart.
...
- **Implemented** (ADR-0014) — the first writer is the Phase 2 validation workflow: `POST /locations`, `PUT /locations/{id}`, and per-row `POST /locations/import` land here instead of applying directly when the submitter's organization has `require_review_for_submissions = true` and the submitter isn't an admin. `entity_id = NULL` means "propose a new location"; `entity_id` set means "propose changes to this existing location," with `proposed_changes` holding only the changed fields. Approving a queued entry replays the create/update as the original submitter (so `update_log` attributes it correctly), not as the reviewer. **A second writer, the Market Refresh Engine (ADR-0020), was added in Phase 3**: its proposals have `submitted_by = NULL` (system-sourced, not tied to any user/org) rather than a human submitter, which required widening `list_queue`'s org-scoping join and the approve-route's ownership check to treat a `NULL` submitter as visible to and actionable by every organization's admins — see ADR-0020 point 5.
...
**refresh_runs** — one row per "Refresh Market" invocation. **Implemented** (ADR-0004 design, ADR-0020 implementation).
- `id` (PK), `started_at`, `completed_at` (nullable), `status` (`running`/`completed`/`failed`), `triggered_by` (FK → users, nullable — null for a future scheduled run, always set for v1's manual button), `locations_reviewed`, `changes_queued`, `providers_used` (JSONB list of provider slugs actually run, e.g. `["openstreetmap", "census"]`), `error_message` (nullable)

A refresh run never writes to `locations` directly — it only creates `validation_queue` entries, one per location with any drifted field, combining every changed field into a single proposal (ADR-0020 point 3) rather than one entry per field. Approving a queue entry is what writes to `locations` and to `update_log` (with `change_source = "verification"`), per [ARCHITECTURE.md](ARCHITECTURE.md#market-refresh-engine-adr-0004). v1's providers are OpenStreetMap (address-drift, via reverse-geocoding) and US Census (demographics) only — new-competitor-POI, closed/moved-business, and duplicate-location detection from ADR-0004's original design were explicitly deferred; see ADR-0020 point 1 for why.
